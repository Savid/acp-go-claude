...
.PHONY: build lint fmt-check fmt test coverage-check test-cross-compile test-integration-smoke test-integration-live test-integration-attended test-integration-keystore test-integration-cover test-integration-native-browser docs-audit clean tidy vuln vuln-sarif modernize-check audit test/cover help
...
test-cross-compile:
	...
	GOOS=freebsd GOARCH=amd64 go test -c -o .tmp/cross/claude-freebsd.test ./internal/claude
	GOOS=openbsd GOARCH=amd64 go test -c -o .tmp/cross/claude-openbsd.test ./internal/claude
