...
test-integration-native-browser:
	...
	{ (set -eu; export ACP_GO_CLAUDE_RUN_LIVE_TOKENS=0 ACP_GO_CLAUDE_RUN_ATTENDED=0 ACP_GO_CLAUDE_RUN_KEYSTORE=0 ACP_GO_CLAUDE_RUN_INTEGRATION=1; case "$$(uname -m)" in x86_64) goarch=amd64; platform=linux/amd64 ;; arm64|aarch64) goarch=arm64; platform=linux/arm64 ;; *) echo "unsupported native-browser architecture: $$(uname -m)" >&2; exit 1 ;; esac; \
...
test-integration-smoke:
	ACP_GO_CLAUDE_RUN_LIVE_TOKENS=0 ACP_GO_CLAUDE_RUN_ATTENDED=0 ACP_GO_CLAUDE_RUN_KEYSTORE=0 ACP_GO_CLAUDE_RUN_INTEGRATION=1 go test -race -count=1 -tags=integration -timeout=300s -parallel=4 -v ./integration/...

## test-integration-live: run full live integration tests
test-integration-live:
	ACP_GO_CLAUDE_RUN_ATTENDED=0 ACP_GO_CLAUDE_RUN_KEYSTORE=0 ACP_GO_CLAUDE_RUN_INTEGRATION=1 ACP_GO_CLAUDE_RUN_LIVE_TOKENS=1 go test -race -count=1 -tags=integration -timeout=900s -parallel=4 -v ./integration/...
...
test-integration-attended:
	...
	dir=$$(cd "$$dir" && pwd); \
	export ACP_GO_CLAUDE_RUN_INTEGRATION=1 ACP_GO_CLAUDE_RUN_ATTENDED=1 ACP_GO_CLAUDE_RUN_LIVE_TOKENS=0 ACP_GO_CLAUDE_RUN_KEYSTORE=0; \
...
test-integration-keystore:
	ACP_GO_CLAUDE_RUN_LIVE_TOKENS=0 ACP_GO_CLAUDE_RUN_ATTENDED=0 ACP_GO_CLAUDE_RUN_INTEGRATION=1 ACP_GO_CLAUDE_RUN_KEYSTORE=1 go test -race -count=1 -tags=integration -timeout=900s -v -run TestKeystore ./...
...
test-integration-cover:
	...
	mkdir "$$dir/data"; \
	go build -cover -coverpkg=./... -o "$$dir/acp-go-claude" ./cmd/acp-go-claude; \
	{ status=0; ACP_GO_CLAUDE_RUN_LIVE_TOKENS=0 ACP_GO_CLAUDE_RUN_ATTENDED=0 ACP_GO_CLAUDE_RUN_KEYSTORE=0 ACP_GO_CLAUDE_RUN_INTEGRATION=1 ACP_GO_CLAUDE_AGENT_BINARY="$$dir/acp-go-claude" GOCOVERDIR="$$dir/data" go test -race -count=1 -tags=integration -timeout=600s -parallel=4 -v ./integration/... 2>&1 || status=$$?; echo "$$status" >"$$dir/status"; } | tee "$$dir/output"; \
