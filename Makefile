...
docs-audit:
	...
	@for flag in -path -home -scratch-dir -provider-auth-root -provider-auth-direct-home -model -debug -version -seed-file -claude-bare -claude-hide-auth -claude-permission-mode -claude-settings-file -claude-system-prompt; do rg -q -- "$$flag" docs/reference/cli.mdx cmd/acp-go-claude/main.go || { echo "missing CLI flag in docs/code: $$flag"; exit 1; }; done
