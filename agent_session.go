...
func (a *Agent) validateStart(params sessionStart, mcpServers []acp.McpServer, meta map[string]any) (sessionStart, error) {
	...
	canonical, pathErr := filepath.EvalSymlinks(params.cwd)
	if pathErr != nil {
		return sessionStart{}, wire.Unsupported("cwd")
	}
...
func (a *Agent) newSession(start sessionStart) *session {
	...
	s.agentDir = claude.ConfigDir(a.options.Home, func(key string) (string, bool) { return process.Lookup(env, key) })
...
// establishing response is on the wire: the lifecycle snapshot, then the catalog.
func (s *session) publishOpen(ctx context.Context) error {
	if err := s.openStream(ctx); err != nil {
		return err
	}

	return s.publishCommands(ctx)
}
...
func inheritCarrier(meta sessionMeta, record sessionRecord) ClaudeOptions {
	...
	if !meta.options.bareSet {
		options.Bare = record.Bare
	}

	if options.SystemPrompt == "" {
		options.SystemPrompt = record.SystemPrompt
	}
...
// Prompt sends one turn to claude and streams updates until it settles.
...
// session with the same id. Native state stays in claude's home.
