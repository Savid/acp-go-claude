...
func TestRunContainmentDarwinGoldenJSON(t *testing.T) {
	...
	wantDiagnose := fmt.Sprintf(`{"vendor":"claude","containment":"best_effort","scratch_parent":%q,"warning":%q,"records":[]}`+"\n", parent, warning)
	...
	root := filepath.Join(parent, "acp-go-claude-runtime-partial")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	generation, err := nativeclaude.NewDarwinGenerationRecord(parent, root, "prompt")
	if err != nil {
		t.Fatal(err)
	}
	...
	if code != 1 || !strings.HasPrefix(stderr.String(), "acp-go-claude: correlated or ambiguous processes remained") {
		t.Fatalf("partial cleanup = %d, stderr=%q", code, stderr.String())
	}
	wantCleanup := fmt.Sprintf(`{"vendor":"claude","containment":"best_effort","scratch_parent":%q,"warning":%q,"runtime_id":%q,"generation_root":%q,"term_signaled_pids":[],"kill_signaled_pids":[],"remaining_correlated_pids":[],"ambiguous_pids":[%d],"root_removed":false}`+"\n",
...
func TestRunContainmentCleanupOperationalFailure(t *testing.T) {
	...
	if code != 1 || !strings.HasPrefix(stderr.String(), "acp-go-claude: ") || strings.Count(strings.TrimSpace(stderr.String()), "\n") != 0 {
