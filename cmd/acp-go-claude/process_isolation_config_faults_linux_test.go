...
const isolationConfigCovPolicyJSON = `{"uid":20001,"gid":20002,` +
	...
	`"inheritEnvironment":["ANTHROPIC_API_KEY"],` +
...
func TestValidateProcessIsolationConfigRefusesEveryUnprovenField(t *testing.T) {
	...
	for name, testCase := range map[string]struct {
		...
		"relative state root": {
			...
			wantError: "standaloneStateRoot must be a canonical absolute path of at most 4096 bytes",
		},
		"state root with a control character": {
			mutate:    func(config *processIsolationConfig) { config.StandaloneStateRoot = "/var/lib/acp\tcov" },
			wantError: "standaloneStateRoot must not contain control characters",
		},
		"state root inside the authority root": {
			mutate: func(config *processIsolationConfig) {
				config.StandaloneStateRoot = "/var/lib/acp-go/agent-identities/claude/cov"
			},
			wantError: "standaloneStateRoot must be outside the authority root",
		},
		"state root is the authority root itself": {
			mutate: func(config *processIsolationConfig) {
				config.StandaloneStateRoot = "/var/lib/acp-go/agent-identities"
			},
			wantError: "standaloneStateRoot must be outside the authority root",
		},
		"unknown uid": {
		...
		"inherited name also declared in base": {
			mutate: func(config *processIsolationConfig) {
				config.BaseEnvironment["ANTHROPIC_API_KEY"] = "policy-secret"
				config.InheritEnvironment = []string{"ANTHROPIC_API_KEY"}
			},
			wantError: `environment variable "ANTHROPIC_API_KEY" appears in both baseEnvironment and inheritEnvironment`,
		},
		"inherited name is unset": {
			mutate:    func(config *processIsolationConfig) { config.InheritEnvironment = []string{"OPENAI_API_KEY"} },
			wantError: `inheritEnvironment variable "OPENAI_API_KEY" is unset`,
		},
		"inherited value carries NUL": {
			...
			wantError: `inheritEnvironment variable "ANTHROPIC_API_KEY" contains NUL`,
		},
...
func TestLoadProcessIsolationConfigAcceptsOnlyABoundedTrustedPolicy(t *testing.T) {
	...
	if config.UID != 20001 || config.GID != 20002 || config.StandaloneOwnerID != "cov-owner" ||
		config.BaseEnvironment["ANTHROPIC_API_KEY"] != "explicit-secret" || config.InheritEnvironment != nil {
...
func TestProcessIsolationValueValidatorsBoundTheirInputs(t *testing.T) {
	...
	for _, value := range []string{"/var/lib/acp-cov", "/a", "/var/lib/acp-go/agent-identities-other"} {
	...
	for name, testCase := range map[string]struct {
		...
		"authority root":  {"/var/lib/acp-go/agent-identities", "outside the authority root"},
		"inside authority root": {
			"/var/lib/acp-go/agent-identities/claude", "outside the authority root",
		},
	} {
		t.Run(name, func(t *testing.T) {
