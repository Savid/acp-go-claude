...
func ExampleNewClaudeOptions() {
	options := claudeacp.NewClaudeOptions(
		claudeacp.WithClaudeModel("haiku"),
		claudeacp.WithClaudeEffort("high"),
		claudeacp.WithClaudePermissionMode("plan"),
	)

	fmt.Println(options.Model)
	fmt.Println(options.Effort)
	fmt.Println(options.PermissionMode)
	// Output:
	// haiku
	// high
	// plan
...
func ExampleServe_initialize() {
	...
	go func() {
		done <- claudeacp.Serve(ctx, clientToAgentReader, agentToClientWriter)
	}()
