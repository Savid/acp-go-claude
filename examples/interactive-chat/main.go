...
const agentPackage = "github.com/savid/acp-go-claude/cmd/acp-go-claude"
...
func (ui *chatUI) upsertMessageBlockLocked(display *messageDisplay) {
	lines := prefixedLines("claude", []string{display.text})
	if len(lines) == 0 {
		return
	}
...
func runInteractiveLoop(
	...
	for {
		...
		case <-ctx.Done():
			if running {
				_ = conn.Cancel(context.Background(), claudeacp.CancelRequest(sessionID, activeTurnNonce))
			}
...
func handleInputEvent(
	...
	case inputPrompt:
		...
		if quitCommand(prompt) {
			if running {
				_ = conn.Cancel(context.Background(), claudeacp.CancelRequest(sessionID, turnNonce))
			}
	...
	case inputInterrupt:
		...
		if err := conn.Cancel(ctx, claudeacp.CancelRequest(sessionID, turnNonce)); err != nil {
...
func runPrompt(
	...
	resp, err := conn.Prompt(ctx, claudeacp.TextPromptRequest(sessionID, turnNonce, prompt))
	if err != nil {
		if errors.Is(err, context.Canceled) {
