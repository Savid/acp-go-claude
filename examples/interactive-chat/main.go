...
func run(ctx context.Context, args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) int {
	...
	model := flags.String("model", "", "model for the session by native identifier")
	...
	go func() {
		served <- claudeacp.Serve(serveCtx, agentReader, agentWriter, claudeacp.WithLogger(slog.New(slog.DiscardHandler)))
	}()
...
func chat(ctx context.Context, conn agentConnection, input *bufio.Reader, cwd string, model string, stdout io.Writer) error {
	...
	var opts []claudeacp.SessionRequestOption
	if model != "" {
		opts = append(opts, claudeacp.WithSessionClaudeOptions(claudeacp.NewClaudeOptions(claudeacp.WithClaudeModel(model))))
	}

	session, err := conn.NewSession(ctx, claudeacp.NewSessionRequest(cwd, opts...))
	if err != nil {
		return err
	}
	...
	for {
		...
		if _, err := conn.Prompt(ctx, claudeacp.TextPromptRequest(session.SessionId, text)); err != nil {
			return err
		}
