// Command minimal-client runs acp-go-claude as a subprocess, creates one session,
...
func converse(ctx context.Context, conn agentConnection, cwd string, prompt string, stdout io.Writer) error {
	...
	resp, err := conn.Prompt(ctx, claudeacp.TextPromptRequest(session.SessionId, prompt))
	if err != nil {
		return err
	}
