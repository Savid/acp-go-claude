...
func TestRunUsesInferredValuesAndLoadedSession(t *testing.T) {
	...
	expectedHome := "/home/claude"
	runLoaded = func(_ context.Context, store claudeacp.SessionStore, sessionID string, gotCwd string, prompt string, claudePath string, claudeHome string, stdout io.Writer) error {
		require.Equal(t, expectedSessionID, sessionID)
		require.Equal(t, expectedCwd, gotCwd)
		require.Equal(t, expectedPrompt, prompt)
		require.Equal(t, expectedPath, claudePath)
		require.Equal(t, expectedHome, claudeHome)
		entries, err := store.Load(context.Background(), claudeacp.SessionKey{SessionID: sessionID})
		require.NoError(t, err)
		require.Len(t, entries, 2)
	...
	err := run(context.Background(), []string{"-file", path, "-prompt", "prompt", "-path", "/bin/claude", "-home", "/home/claude"}, &stdout, io.Discard)
	require.NoError(t, err)
...
func TestRunErrors(t *testing.T) {
	...
	runLoaded = func(context.Context, claudeacp.SessionStore, string, string, string, string, string, io.Writer) error {
		return nil
	}
	t.Cleanup(func() {
		getwd = previousGetwd
	...
	require.NoError(t, os.WriteFile(path, []byte(`{"type":"assistant","sessionId":"session-1"}`+"\n"), 0o600))
	err = run(context.Background(), []string{"-file", path}, io.Discard, io.Discard)
...
func TestRunLoadedSessionWithFakeServe(t *testing.T) {
	...
	var stdout bytes.Buffer
	err := runLoadedSession(context.Background(), claudeacp.NewInMemorySessionStore(), "session-1", t.TempDir(), "prompt", "", "", &stdout)
	require.NoError(t, err)
...
func TestRunLoadedSessionErrors(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := runLoadedSession(context.Background(), claudeacp.NewInMemorySessionStore(), "session-1", t.TempDir(), "prompt", "", "", io.Discard)
			require.Error(t, err)
		})
	}
}

func fakeServe(t *testing.T, failMethod string, failErr *acp.RequestError) func(context.Context, io.Reader, io.Writer, ...claudeacp.Option) error {
	t.Helper()

	return func(ctx context.Context, input io.Reader, output io.Writer, _ ...claudeacp.Option) error {
