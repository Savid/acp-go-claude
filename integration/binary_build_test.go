...
var buildIntegrationBinary = sync.OnceValues(func() (string, error) {
	dir, err := os.MkdirTemp("", "acp-go-claude-integration-")
	if err != nil {
		return "", err
	}
	binaryBuildDir = dir
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, "go", "build", "-o", binary, "./cmd/acp-go-claude")
...
func integrationBinaryPath(t *testing.T) string {
	t.Helper()
	if binary := os.Getenv("ACP_GO_CLAUDE_AGENT_BINARY"); binary != "" {
