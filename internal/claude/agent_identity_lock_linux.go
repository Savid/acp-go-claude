...
func duplicateAgentIdentityLock(file *os.File) (*os.File, error) {
	...
	return os.NewFile(uintptr(fd), "claude-agent-identity-lock"), nil
}
...
func validateTurnSupervisorAdoptedAuthorityDisposition(
	...
	case turnSupervisorBorrowed:
		return validateBorrowedAgentIdentityDisposition(
			config.Isolation.UID, config.Isolation.GID, testOnly, testRoot,
		)
