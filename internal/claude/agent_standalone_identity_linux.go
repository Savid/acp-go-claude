...
func bindAgentStandaloneStateRoot(path string, uid, gid uint32) (agentStandaloneStateRoot, error) {
	...
	fd, err := agentStandaloneOpen("/", unix.O_PATH|unix.O_DIRECTORY|unix.O_CLOEXEC|unix.O_NOFOLLOW, 0)
	if err != nil {
...
var agentStandaloneOpen = unix.Open
...
func replaceAgentStandaloneDomainRecord(directory *os.File, ownerUID, ownerGID uint32, record agentAuthorityDomainRecord) (replaceErr error) {
	...
	if agentErr := agentStandaloneRenameat(int(directory.Fd()), temporary, int(directory.Fd()), "domain.json"); agentErr != nil {
		return agentErr
	}
...
func loadAgentStandaloneOwner(directory *os.File, uid, ownerUID, ownerGID uint32) (agentStandaloneOwner, error) {
	...
	// exactAgentAuthorityFields has already unmarshalled these same bytes whole,
	// which refuses a payload carrying more than one JSON value, so the decode
	// above consumed all of it.
