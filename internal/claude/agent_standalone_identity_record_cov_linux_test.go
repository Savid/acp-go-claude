...
func TestAgentStandaloneCovMarkerRefusesEveryMalformedPayload(t *testing.T) {
	...
	for index := 0; index < 129; index++ {
		manyPaths = append(manyPaths,
			`{"base":"/srv/claude","segments":["p`+strconv.Itoa(index)+`"],"action":"revoke-path","rootDev":1,"rootIno":1}`,
		)
	}
	for _, testCase := range []struct {
		...
		},
		{
			...
			payload: agentStandaloneCovActiveMarker(uid, gid, "k", lease,
				`[{"base":"/srv/claude","segments":["a"],"action":"revoke-path","rootDev":1}]`,
			),
		...
		},
		{
			...
			payload: agentStandaloneCovActiveMarker(uid, gid, "k", lease,
				`[{"base":"/srv/claude","segments":["a"],"action":"revoke-path","rootDev":1,"rootIno":0}]`,
			),
		...
		},
		{
			...
			payload: agentStandaloneCovActiveMarker(uid, gid, "k", lease,
				`[{"base":"/srv/claude","segments":["a"],"action":"revoke-path","rootDev":1,"rootIno":1},`+
					`{"base":"/srv/claude","segments":["a"],"action":"revoke-tree","rootDev":1,"rootIno":1}]`,
			),
			want: `marker path "/srv/claude/a" is duplicated with actions "revoke-path" and "revoke-tree"`,
		},
		{
			name: "removal overlapping a retained subtree",
			payload: agentStandaloneCovActiveMarker(uid, gid, "k", lease,
				`[{"base":"/srv/claude","segments":["a"],"action":"remove-path","rootDev":1,"rootIno":1},`+
					`{"base":"/srv/claude","segments":["a","b"],"action":"revoke-path","rootDev":1,"rootIno":1}]`,
			),
			want: `marker removal path "/srv/claude/a/b" conflicts with "/srv/claude/a"`,
		},
	} {
...
func TestAgentStandaloneCovMarkerAcceptsDisjointManifestPaths(t *testing.T) {
	...
	agentStandaloneCovWriteRegistryFile(t, directory, "62303.quarantine",
		agentStandaloneCovActiveMarker(uid, gid, "disjoint", "0123456789abcdef0123456789abcdef",
			`[{"base":"/srv/claude","segments":["a"],"action":"remove-path","rootDev":7,"rootIno":8},`+
				`{"base":"/srv/claude","segments":["b"],"action":"revoke-tree","rootDev":7,"rootIno":8}]`,
		),
	)
...
func TestAgentStandaloneCovManifestPathValidation(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			name: "unknown action",
			path: agentStandaloneManifestPath{
				Base: "/srv/claude", Segments: []string{"a"}, Action: "delete-everything", RootDev: 1, RootIno: 2,
			},
...
func TestAgentStandaloneCovOwnerRecordRefusesNonCanonicalAndInvalidBindings(t *testing.T) {
	...
	canonical := agentStandaloneOwner{
		...
		StateRoot: agentStandaloneStateRoot{Path: "/srv/claude/canonical", Dev: 5, Ino: 6},
	}
	...
	controlStateRoot.StateRoot.Path = "/srv/claude/\x01"
	...
	for _, testCase := range []struct {
		...
		{
			name:    "missing field",
			payload: `{"version":1,"uid":62311,"gid":62312,"kind":"standalone-provider","provider":"github.com/savid/acp-go-claude","ownerId":"x"}` + "\n",
		...
		},
		{
			...
			payload: `{"version":1,"uid":62311,"gid":62312,"kind":"standalone-provider",` +
				`"provider":"github.com/savid/acp-go-claude","ownerId":"x",` +
				`"stateRoot":{"path":"/srv/claude/x","dev":1,"ino":2,"extra":3}}` + "\n",
		...
		},
		{
			...
			payload: `{"version":1,"uid":"62311","gid":62312,"kind":"standalone-provider",` +
				`"provider":"github.com/savid/acp-go-claude","ownerId":"x",` +
				`"stateRoot":{"path":"/srv/claude/x","dev":1,"ino":2}}` + "\n",
