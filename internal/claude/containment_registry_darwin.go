...
const (
	darwinRegistryDir        = "acp-go-claude-containment"
...
func createDarwinRecord(parent string, record darwinContainmentRecord) error {
	...
	for _, entry := range entries {
		...
		if now.Sub(info.ModTime()) > darwinRecordLifetime {
			expired, readErr := readDarwinRecordFile(parent, registry, entry.Name())
			if readErr != nil {
				return fmt.Errorf("inspect expired containment record: %w", readErr)
			}

			if expired.State != darwinStateGroupAbsent {
				count++
...
func CleanupDarwinContainment(scratchParent, runtimeID string, force bool, output io.Writer) error { //nolint:gocyclo // Operator cleanup owns one deadline-terminal control graph.
	if !force {
	...
	if !darwinCleanupNow().Before(deadline) {
		return errors.New("containment cleanup deadline reached during registry enumeration")
	}
	...
	if !darwinCleanupNow().Before(deadline) {
		return errors.New("containment cleanup deadline reached before root validation")
	}

	validatedRoot, rootExists, rootIdentity, err := inspectValidatedDarwinGenerationRoot(parent, record.GenerationRoot)
	if err != nil {
		return err
	}

	if !darwinCleanupNow().Before(deadline) {
		return errors.New("containment cleanup deadline reached during root validation")
	}

	if !rootExists && record.State != darwinStateGroupAbsent {
	...
	finish := func(remaining []DarwinContainmentCandidate, operationallyIncomplete bool) error {
		ambiguousPIDs := sortedPIDSet(cleanup.ambiguous)
		rootRemoved := false

		if !operationallyIncomplete && rootExists && len(remaining) == 0 && len(ambiguousPIDs) == 0 {
			if !darwinCleanupNow().Before(cleanup.deadline) {
				operationallyIncomplete = true
			} else {
				currentRoot, currentExists, currentIdentity, identityErr := inspectValidatedDarwinGenerationRoot(parent, record.GenerationRoot)
				switch {
				case !darwinCleanupNow().Before(cleanup.deadline):
					operationallyIncomplete = true
				case identityErr != nil:
					return fmt.Errorf("revalidate selected containment root: %w", identityErr)
				case !currentExists || currentRoot != validatedRoot || !os.SameFile(rootIdentity, currentIdentity):
					return errors.New("selected containment root identity changed before removal")
				}
			}

			if !operationallyIncomplete {
				if removeErr := darwinRemoveGenerationRoot(validatedRoot); removeErr != nil {
		...
		if operationallyIncomplete {
			return errors.New("containment cleanup deadline reached before completion")
		}
	...
	for _, pid := range ambiguousDarwinRecordedIdentity(record) {
	...
	if !darwinCleanupNow().Before(deadline) {
		...
		return finish(preKillCandidates, true)
	}
	...
	if !darwinCleanupNow().Before(deadline) {
		...
		return finish(killCandidates, true)
	}
	...
	return finish(remaining, cleanup.expired)
}
...
func readDarwinRecords(scratchParent, runtimeID string) (string, []darwinContainmentRecord, error) {
	...
	for _, entry := range entries {
		...
		record, readErr := readDarwinRecordFile(parent, registry, entry.Name())
		if readErr != nil {
			return "", nil, readErr
		}
...
func readDarwinRecordFile(parent, registry, name string) (darwinContainmentRecord, error) {
	recordPath := filepath.Join(registry, name)
	...
	if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != 0o600 {
		return darwinContainmentRecord{}, fmt.Errorf("containment record %s is not a mode-0600 regular file", name)
	}
	...
	if statErr != nil || !openedInfo.Mode().IsRegular() || openedInfo.Mode().Perm() != 0o600 {
		...
		return darwinContainmentRecord{}, errors.Join(fmt.Errorf("containment record %s is not a mode-0600 regular file", name), statErr)
	}
	...
	if !validCurrentDarwinRecord(parent, strings.TrimSuffix(name, ".json"), record) {
		return darwinContainmentRecord{}, fmt.Errorf("invalid current-format containment record %s", name)
	}
...
func validCurrentDarwinRecord(parent, filenameID string, record darwinContainmentRecord) bool {
	if record.SchemaVersion != darwinRecordFormat || record.Vendor != darwinVendor || record.Containment != darwinBestEffortMode ||
		...
		!strings.HasPrefix(filepath.Base(record.GenerationRoot), "acp-go-claude-runtime-") ||
...
func validDarwinLifecycle(lifecycle string) bool {
	switch lifecycle {
	case "runtime", windowSession, darwinLifecyclePrompt, "discovery":
		return true
	default:
		return false
	}
...
func validatedDarwinGenerationRoot(parent, root string) (string, bool, error) {
	validatedRoot, exists, _, err := inspectValidatedDarwinGenerationRoot(parent, root)

	return validatedRoot, exists, err
}

func inspectValidatedDarwinGenerationRoot(parent, root string) (string, bool, os.FileInfo, error) {
	if !strings.HasPrefix(filepath.Base(root), "acp-go-claude-runtime-") {
		return "", false, nil, errors.New("selected containment root has an invalid generation prefix")
	}

	resolvedParent, err := darwinEvalSymlinks(parent)
	if err != nil {
		return "", false, nil, fmt.Errorf("resolve selected scratch parent: %w", err)
	}
	...
	if errors.Is(err, os.ErrNotExist) {
		...
		if absoluteErr != nil || !pathWithin(parent, absoluteRoot) {
			return "", false, nil, errors.New("selected containment root is outside the scratch parent")
		}

		return absoluteRoot, false, nil, nil
	}

	if err != nil {
		return "", false, nil, fmt.Errorf("inspect selected containment root: %w", err)
	}

	if !info.IsDir() || info.Mode()&os.ModeSymlink != 0 {
		return "", false, nil, errors.New("selected containment root is not a real directory")
	}

	resolvedRoot, err := darwinEvalSymlinks(root)
	if err != nil {
		return "", false, nil, fmt.Errorf("resolve selected containment root: %w", err)
	}
	...
	return resolvedRoot, true, info, nil
}
