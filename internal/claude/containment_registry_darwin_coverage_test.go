...
func TestDarwinRegistryCreationAndGenerationFailures(t *testing.T) {
	...
	parent := t.TempDir()
	outside := filepath.Join(t.TempDir(), "acp-go-claude-runtime-outside")
	...
	if _, err := NewDarwinGenerationRecord(parent, filepath.Join(parent, "acp-go-claude-runtime-random"), "prompt"); err == nil || !strings.Contains(err.Error(), "runtime id") {
	...
	if _, err := NewDarwinGenerationRecord(parent, filepath.Join(parent, "acp-go-claude-runtime-identity"), "prompt"); err == nil || !strings.Contains(err.Error(), "wrapper") {
	...
	if _, err := NewDarwinGenerationRecord(parentFile, filepath.Join(parentFile, "acp-go-claude-runtime-create"), "prompt"); err == nil {
		t.Fatal("file scratch parent was accepted")
	}

	root := filepath.Join(parent, "acp-go-claude-runtime-start")
...
func TestDarwinRegistryMaintenanceAndAtomicWriterBranches(t *testing.T) {
	...
	expired := validDarwinTestRecord(parent, strings.Repeat("f", 32))
	expired.State = darwinStateGroupAbsent
	if err := writeDarwinRecordAtomic(registry, expired, true); err != nil {
		t.Fatal(err)
	}
	old := filepath.Join(registry, expired.RuntimeID+".json")
...
func TestDarwinRegistryRetainsUnresolvedExpiredRecords(t *testing.T) {
	parent := t.TempDir()
	...
	oldTime := time.Now().Add(-darwinRecordLifetime - time.Hour)
	for runtimeID, state := range map[string]string{
		strings.Repeat("d", 32): darwinStateRunning,
		strings.Repeat("e", 32): darwinStateIncomplete,
	} {
		unresolved := validDarwinTestRecord(parent, runtimeID)
		unresolved.State = state
		if err := writeDarwinRecordAtomic(registry, unresolved, true); err != nil {
			t.Fatal(err)
		}
	...
	for _, runtimeID := range []string{strings.Repeat("d", 32), strings.Repeat("e", 32)} {
		if _, err := os.Stat(filepath.Join(registry, runtimeID+".json")); err != nil {
			t.Fatalf("unresolved expired record %s was removed: %v", runtimeID, err)
		}
	}

	invalidExpired := filepath.Join(registry, strings.Repeat("c", 32)+".json")
	if err := os.WriteFile(invalidExpired, []byte("{}\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.Chtimes(invalidExpired, oldTime, oldTime); err != nil {
		t.Fatal(err)
	}
	record.RuntimeID = strings.Repeat("a", 32)
	if err := createDarwinRecord(parent, record); err == nil || !strings.Contains(err.Error(), "inspect expired") {
		t.Fatalf("invalid expired record error = %v", err)
	}
}
...
func TestDarwinGenerationRootAndProcessCorrelationEdges(t *testing.T) {
	...
	if _, _, err := validatedDarwinGenerationRoot(filepath.Join(parent, "missing-parent"), filepath.Join(parent, "acp-go-claude-runtime-missing")); err == nil {
		t.Fatal("missing parent accepted")
	}
	outside := filepath.Join(t.TempDir(), "acp-go-claude-runtime-missing")
	if _, _, err := validatedDarwinGenerationRoot(parent, outside); err == nil {
		t.Fatal("missing outside root accepted")
	}
	file := filepath.Join(parent, "acp-go-claude-runtime-file")
	...
	realRoot := filepath.Join(parent, "acp-go-claude-runtime-real")
	if err := os.Mkdir(realRoot, 0o700); err != nil {
		t.Fatal(err)
	}
	link := filepath.Join(parent, "acp-go-claude-runtime-link")
...
func TestDarwinRegistrySyscallErrorSeams(t *testing.T) { //nolint:gocyclo // One table-like seam audit keeps global restoration atomic.
	...
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-claude-runtime-seam")
	absCalls := 0
	...
	darwinRegistryReadDir = originalReadDir
	expired := validDarwinTestRecord(parent, strings.Repeat("8", 32))
	expired.State = darwinStateGroupAbsent
	if writeErr := writeDarwinRecordAtomic(registry, expired, true); writeErr != nil {
		t.Fatal(writeErr)
	}
	expiredPath := filepath.Join(registry, expired.RuntimeID+".json")
	oldTime := time.Now().Add(-darwinRecordLifetime - time.Hour)
	...
	darwinRecordAbs = func(string) (string, error) { return "", want }
	if _, _, err := validatedDarwinGenerationRoot(parent, filepath.Join(parent, "acp-go-claude-runtime-missing")); err == nil {
		t.Fatalf("missing-root abs error = %v", err)
	}
	restore()
	darwinRegistryLstat = func(string) (os.FileInfo, error) { return nil, want }
	if _, _, err := validatedDarwinGenerationRoot(parent, filepath.Join(parent, "acp-go-claude-runtime-root")); !errors.Is(err, want) {
		t.Fatalf("root lstat error = %v", err)
	}
	restore()
	evalRoot := filepath.Join(parent, "acp-go-claude-runtime-eval")
...
func TestDarwinRegistryFinalLogicalBranches(t *testing.T) {
	...
	fileRoot := filepath.Join(fileParent, "acp-go-claude-runtime-file")
	...
	linkedRoot := filepath.Join(link, "acp-go-claude-runtime-outside")
...
func validDarwinTestRecord(parent, runtimeID string) darwinContainmentRecord {
	return darwinContainmentRecord{
		...
		GenerationRoot: filepath.Join(parent, "acp-go-claude-runtime-"+runtimeID[:4]),
...
func newDarwinCoverageGeneration(t *testing.T, suffix string) (string, string, *DarwinGeneration) {
	t.Helper()
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-claude-runtime-"+suffix)
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	generation, err := NewDarwinGenerationRecord(parent, root, "prompt")
