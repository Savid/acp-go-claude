...
// The generated-tree handoff re-reads every descriptor it is about to trust, so
// each syscall it depends on is reached through a seam. Faulting a seam is the
// only way to prove these paths fail closed when the kernel stops answering for
// a descriptor they already accepted, and the only way to stage a second read
...
func validateGeneratedNativeAncestor(
	...
	// The whole ancestry belongs to the trusted supervisor. An explicit policy
	// is the only caller here, and it keeps the supervisor a distinct trusted
	// root identity, so there is no shape in which a foreign owner is safe.
